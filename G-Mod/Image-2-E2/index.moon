	...
	if table.Count(ARG) == 0 -- Checking if input CMD == valid
		...
		req_onSuccess = ( body, len, headers, code ) ->
			...
			if SCR == "Bad image type.  Please enter a URL to a GIF, JPG, JPEG, or PNG"
				...
				O ..= "Checked=0,propSpawnUndo(0),propSpawnEffect(0)"
				...
				O ..= "\nif(clk(\"build_tm\")) {"
				O ..= "\tI++,build(I),"
				O ..= "\tif(I<=#{PX}) { timer(\"build_tm\",#{SPEED}),print(_HUD_PRINTCENTER,\"Image: \"+toString((I/#{PX})*100)+\"% Built\"),setName(\"Image: \"+toString((I/#{PX})*100)+\"% Built\") }"
				O ..= "\telse {"
				...
				-- /Checkup
				O ..= "} elseif(keyClk()) {\tlocal AE=owner():aimEntity(),\tlocal KeyP=keyClkPressed(),\tif(KeyP==\"e\"&&AE==entity()&&Checked==0) {\t\tChecked = 1,\t\tfor (I2=1, #{PX}) {"
				O ..= "\t\t\tif("
				if PROP == "0" then O..="holoEntity(I2)" else if PROP == "1" then O..="Ents[I2,array][2,entity]"
				O ..= ") {"
				-- O ..= "\t\t\tFEnt = Ents[1,array][2,entity]\n\t\t\tPEnt = Ents[I2-1,array][2,entity]\n\t\t\tEnt  = Ents[I2,array][2,entity]\n\t\t\tAEnt = Ents[I2+1,array][2,entity]\n\t\t\tprint(_HUD_PRINTTALK,\"Pixel#\"+I2+\" was found as missing! Attempting to replace!\")\n\t\t\tbuild(I2)\n\t\t\tif(I2 > 1) {\n\t\t\t\tif(I2 == 1) { weld(FEnt,Ent) }\n\t\t\t\tweld(AEnt,Ent), weld(PEnt,Ent)\n\t\t\t}"
				O ..= "\t\t\tFEnt = Ents[1,array][2,entity],\t\t\tPEnt = Ents[I2-1,array][2,entity],\t\t\tEnt  = Ents[I2,array][2,entity],\t\t\tAEnt = Ents[I2+1,array][2,entity],\t\t\tprint(_HUD_PRINTTALK,\"Pixel#\"+I2+\" was found as missing! Attempting to replace!\"),\t\t\tbuild(I2)"
				O ..= "\t\t\t}"
				O ..= "\t\t}"
				O ..= "\t}"
				-- \Checkup
